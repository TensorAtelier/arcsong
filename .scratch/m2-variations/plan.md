...
- Real Finalize check (`ARCSONG_REAL_ENGINE=1 … -k finalized`, 2026-09-17, on AC): an 8-step Draft of the short clip was finalized in one server run (38 s total); the Final's `semantic.npy`, `noise.npy` and `score.abc` are byte-identical to the Draft's and its latents differ.
...
- `arcsong serve --engine fake` now writes 20 s tones (was 1 s), long enough to compare Takes in the page. Tests keep the 1 s default.
