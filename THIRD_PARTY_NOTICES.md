...
`web/public/soundfont/acoustic_grand_piano-mp3.js` (served from `songloom/static/soundfont/`)
is taken from [midi-js-soundfonts](https://github.com/gleitz/midi-js-soundfonts) by Benjamin
Gleitzman (MIT licence), which packages samples from the FluidR3 GM soundfont by Frank Wen
(MIT licence). It is vendored so the Score view's melody preview works without reaching the
network; songloom does not load anything from a CDN at runtime.
