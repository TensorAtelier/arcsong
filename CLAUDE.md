...
- Released: [v0.1.0](https://github.com/TensorAtelier/arcsong/releases/tag/v0.1.0), installed with `uv tool install git+https://github.com/TensorAtelier/arcsong`. Cutting a release: `docs/release.md`. The wheel ships `arcsong` only (never `spike`, which stays in the repository); the suite is run on Pythons 3.12–3.14 (the pin is `>=3.12`, so a newer one installs untested); the README's samples and screenshots live in `docs/media/` with `.gitignore` exceptions.
