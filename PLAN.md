...
Apple Silicon first (via the `mlx-Yue` port); NVIDIA (official `yue2` package) later.
...
| 5 | **Draft → final** — quick 8-step render, then re-synthesize the same take at 32 steps | Cheaper iteration *only if* `synthesize` is a large share of job time — fewer steps don't speed up `generate_semantic`. *Verify feasibility + per-stage time split in M0.* If semantic generation dominates, a minimal plan-first flow ("plan only → approve → render", from #7) is the better iteration lever and moves to v1.1. | v1 if M0 confirms, else v1.1 |
...
**v1 = #1–#4 (+#5 if cheap).** Definition of usable: a fresh clone → one install command →
...
- **Server ↔ worker IPC:** `multiprocessing` queues for jobs and events, plus a shared
  ...
  period, kill the worker and restart it (accept the reload cost).
- **Progress throttling:** `on_token` fires per token; the worker coalesces events to a few
  per second before they reach SSE.
- **Restart semantics:** on server start, jobs left `running` become `failed`; `queued` jobs
  resume in order.
- **Memory-aware defaults:** detect total RAM; default to 8-bit precision on small machines
  (e.g. ≤24 GB) and show a clear warning instead of an out-of-memory crash. Document the
  minimum RAM.
...
- **Use the staged API, not the CLI.** `lyra.YuE2Pipeline` exposes
  `plan → generate_semantic → synthesize → decode`, each with `cancelled=` and
  progress/`on_token` callbacks. That gives per-stage progress bars, real cancellation, and
  the "edit plan then render" flow (#7) for free.
...
- **Library = artifact dirs on disk + SQLite index.** Don't invent a format: store whatever
  ...
  result.json). The `.npy` files are what make replay / draft→final possible. They may be
  much larger than the FLAC (size measured in M0), so the library shows disk usage and lets
  users delete drafts or clear intermediates once a take is final.
...
### M1b — v1 usable (features #3–#4, maybe #5)
Library (list/filter, player, download, re-run, disk usage + draft cleanup), Setup page
(model download with progress, `doctor`, licence acknowledgement, RAM check).

### M2 — Variations (#6)
"Generate ×N" with distinct seeds, compare view with synced players + waveforms, star/pick,
promote pick to final (32-step) render.
...
`CudaEngine` over official `yue2-infer`; engine auto-detect; test on a CUDA machine;
document Linux/Windows install.
